...
public class DiagramManipulator {
	...
	/*private String parseCreatetStatement(String graph, Statement statement) {
		...
		if(select.getWhere()!=null)
		{
			if(select.getWhere() instanceof EqualsTo){
				...
				graph+=" -> "+createTable+" [color=red label=\"RIGHT TABLE CREATED HAVING KEYS IN LEFT\"]";						

			}
			else{
				graph+= createTable+" -> "+createTable+" [color=red label=\"CREATE\"]";
			}
		}
	...
	private String parseDeleteStatement(String graph, Statement statement) {
		...
		if(select.getWhere()!=null)
		{
			if(select.getWhere() instanceof EqualsTo){
				...
				graph+=" -> "+delTable+" [color=red label=\"RIGHT TABLE DELETED HAVING KEYS IN LEFT\"]";						

			}
			else{
				graph+= delTable+" -> "+delTable+" [color=red label=\"DELETE\"]";
			}
		}
	...
	private String parseInsertStatement(String graph, Statement statement) {
		...
		if(insertStmt.getSelect()!=null)
		{
			...
			if((sel.toString().contains("JOIN"))){
				graph =  parseSelect(graph, sel)+((SubSelect) sel.getFromItem()).getAlias().getName()+  "->" +tableName.toUpperCase() +" [color=red label=\"INSERT INTO\"];";
			}
			else{
				graph+=(((Table)sel.getFromItem()).getName()).toUpperCase()+ " -> "+(insertStmt.getTable().getName()).toUpperCase()+" [color=red label=\"INSERT INTO\"]";
			}
		}
		else
		{
	...
	private String parseUpdateStatement(String graph, Statement statement) {
		...
		if(select.getSelect()!=null){
			...
			graph+=(((Table)sel.getFromItem()).getName()).toUpperCase()+ " -> "+targetTable+" [color=red label=\"UPDATED FROM "+srcTable+" table\"]";						
		}
		else{
			graph+= targetTable + " -> " + targetTable + " [color=red label=\"UPDATE"+" \"]";
		}
		return graph;
	}
		...
		if(plainSelect.getJoins()!=null)
		{
			...
			for (Join joinn : plainSelect.getJoins()) {
				...
				if(joinn.getOnExpression()==null)
				{
					...
					{
						subGraph+=((SubSelect)joinn.getRightItem()).getAlias().getName()+ " [color=red label=\""+joinType+"\"]"+traverseSelect((SubSelect)joinn.getRightItem());

					}
					else if(joinn.getRightItem() instanceof Table)
					{
						subGraph+=(((Table)joinn.getRightItem()).getName()).toUpperCase()+" [color=red label=\""+joinType+"\"]";						
	...
	private String equalsJoin(String graph, PlainSelect plainSelect, Join joinn, String joinType) {
		...
		String localGraph ="subgraph{"+ subgraph+" -> "+subgraph2+" [color=red label=\""+getJoinType(joinn)+"\"]}";						
		return localGraph;
	}
	...
	private String selectWhere(String graph, PlainSelect plainSelect) {
		...
		String localGraph ="subgraph{"+ subgraph+" -> "+subgraph2+" [color=red label=\"SubSelect\"]}";						
	...
	private String generateGraph(List<String> joinslist, List<String> srcTabList, List<String> tarTableList,List<String> stAloneTabList,String action){
		...
		for(int i=0;i<joinslist.size();i++){
			...
			String tempJoin = "[color=red label=\""+joingraph+"\"];";
			joinslist.remove(i);
		...
		if(srcTabList.isEmpty() && tarTableList.isEmpty() && stAloneTabList.isEmpty()){
			...
			if(action.equalsIgnoreCase("I")){
				graph += tarTableList.get(0) + "-> " +tarTableList.get(0) + " [color=red label=\"INSERT INTO\"];";
			}
			else if(action.equalsIgnoreCase("U")){
				graph += tarTableList.get(0) + "-> " +tarTableList.get(0) + " [color=red label=\"UPDATE\"];";
			}
		}
			...
			if(action.equalsIgnoreCase("C")){
				graph += stAloneTabList.get(0) + "-> " +stAloneTabList.get(0) + " [color=red label=\"CREATE\"];";
			}
			else if(action.equalsIgnoreCase("D")){
				graph += stAloneTabList.get(0) + "-> " +stAloneTabList.get(0) + " [color=red label=\"DELETE\"];";
			}
		}
...

		{
			...
			if(!(tarTableList.isEmpty()))	{
				...
				if(action.equalsIgnoreCase("I")){
					graph += srcTabList.get(srcTabList.size()-1) + "-> " +tarTableList.get(0) + " [color=red label=\"INSERT INTO\"];";
				}
				else if(action.equalsIgnoreCase("U")){
					graph += srcTabList.get(srcTabList.size()-1) + "-> " +tarTableList.get(0) + " [color=red label=\"UPDATE\"];";
				}
			}
			...
			if(action.equalsIgnoreCase("C")){
				graph += srcTabList.get(srcTabList.size()-1) + "-> " +stAloneTabList.get(0) + " [color=red label=\"CREATE\"];";
			}
			else if(action.equalsIgnoreCase("D")){
				graph += srcTabList.get(srcTabList.size()-1) + "-> " +stAloneTabList.get(0) + " [color=red label=\"DELETE\"];";
			}
		}
	}
